...
ALTER TABLE ORDERS ADD CONSTRAINT DRODERS_PK PRIMARY KEY(ORDER_ID);
...
-- 2)서브쿼리
-- 박지성의 구매내역만 인라인 뷰(FROM절)로 가져와서 처리
...
SELECT
  ...
  FROM CUSTOMER C INNER JOIN ORDERS O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID
 WHERE C.CUSTOMER_NAME = '박지성'  --WHERE절이 없으면 전체 구매자 정보 확인
 GROUP BY C.CUSTOMER_NAME;

-- 7. 박지성이 구매한 도서를 발간한 출판사(PUBLISHER)의 수를 조회하시오.
SELECT 
       C.CUSTOMER_NAME
     , COUNT(DISTINCT B.PUBLISHER) AS 출판사수  --PUBLISHER라서 DISTINCT넣어줘야함 같은출판사의 책을 샀을 경우도 O
  FROM CUSTOMER C INNER JOIN ORDERS O
...
-- 8. 고객별로 분류하여 각 고객의 이름과 각 고객별 총 구매액을 조회하시오. 주문이력이 없으면 0으로 조회하시오.
-- 외부조인사용
SELECT
       C.CUSTOMER_NAME
...
SELECT
       C.CUSTOMER_ID    -- 동명이인을 회피하기 위해 넣음
     , C.CUSTOMER_NAME
     , COUNT(O.ORDER_ID) AS 구매횟수
  FROM CUSTOMER C LEFT JOIN ORDERS O
    ON C.CUSTOMER_ID = O.CUSTOMER_ID
 GROUP BY C.CUSTOMER_ID, C.CUSTOMER_NAME;   -- SELECT에 C.CUSTOMER_ID조회하기 싫으면 GROUP BY절에다가만 적어도 됨
                                            -- 그러면 만약 동명이인이 있을경우 박지성 3
                                            --                                 박지성 1
                                            -- 이런식으로 분리되서 나오게 됨